...
console.log(`foodList.indexOf('물회') : ${foodList.indexOf('물회')}`);   // 인덱스 오브는 첫 배열 부터 조회               // 0
console.log(`foodList.indexOf('물회', 1) : ${foodList.indexOf('물회', 1)}`);       // 인덱스 오브 , 1은 2번째부터     // 4
...
console.log(`foodList.lastIndexOf('삼겹살') : ${foodList.lastIndexOf('삼겹살')}`);      // -1     없는 값은 -1 출력

console.log(`foodList.includes('물회') : ${foodList.includes('물회')}`);                // true      포함되어 있으면 true
...
console.log(`chineseFood.pop() : ${chineseFood.pop()}`);        // 양장피    팝은 마지막 배열 부터 뺌
...
chickenList.unshift('간장치킨');      // 배열 앞쪽에 추가함
...
const mix = idol1.concat(idol2);   // 합쳐줌 concat
...
// slice(시작인덱스, 종료인덱스)
//slice(start(인덱스의 시작값), end(인덱스의 종료값) end: 요소의 값은 반환하지 않는다.)  start 이상 end 미만
